...
function autoSweep(config, stats) {
    ...
    do {
        ...
        if (restartNeeded) {
            if (config.lastSweepResult.state === "death" && !isGuessingSolver(config.lastSweepResult.solver)) {
    ...
    function executeSweepCycle(config, stats) {
        ...
        if (isRiddle) {
            ...
            idleTime = getIdleTimeForState(config, sweepResult.state);
...
function getBombArray(field) {
    return field.reduce((rowA, rowB) =>
        rowA.concat(rowB.reduce((cellA, cellB) =>
            cellA.concat(cellB.isBomb ? true : false), [])), []);
}
...
function restartVirtualGame(config) {
    ...
    function createExpertGame() {
        return createVirtualGame(30, 16, 99);
    }
}
...
function sweep(fieldToSweep, bombAmount, withGuessing = true, doLog = true) {
    ...
    function onTriState(message, solver, messages, mode, resultState) {
        ...
        messages.forEach(c => { log("->", formatSolver, c); });
